...
namespace ElementalFPS.Combat.Core
{
	...
	public abstract class Firearm : MonoBehaviour, IRangedWeapon
	{
		...
		[SerializeField] protected float maxRange = 2000f;						//maximum range the firearm can accurately aim
		[SerializeField] protected float minRange = 9f;							//minimum range the firearm can aim accurately when not zoomed
		...
		protected BulletPool bulletPool;                        //pool for all bullets, should be static gameObject pool (Parent Pool)
		...
		protected virtual void InitializeBullet(GameObject bullet)
		{
			muzzlePosition = muzzle.GetMuzzlePosition();
			if (fireFromCamera) {
				bullet.transform.position = playerCamera.transform.position;
				bullet.GetComponent<Bullet>().velocity = playerCamera.transform.forward * muzzleVelocity;
			}
			if (!fireFromCamera) {
				bullet.transform.position = muzzlePosition;
				bullet.GetComponent<Bullet>().velocity = GetFireDirection() * muzzleVelocity;
			}
			bullet.transform.rotation = transform.rotation * Quaternion.Euler(-(transform.localRotation.eulerAngles));
